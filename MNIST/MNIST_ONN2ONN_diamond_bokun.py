# knowledge distillation with an ONN TA and student ONN (Diamond or Bokun layer)
...
if __name__ == "__main__":
    ...
    onn_settings = {
        'N': N,
        'eo_settings': {'alpha': 0.1, 'g':0.5 * np.pi, 'phi_b': -1 * np.pi}, # not used
        'topology': 'Clements',
    ...
    for seed in range(1, 20, 2):
        ...
        if np.max(val_accuracy_perfect) >= best_acc:
            ...
            np.save(f"{save_path}/phases_Bokun_KD_LeNet.npy", best_phases_perfect)
    ...
    save_dict_to_excel(save_dict, f"{save_path}/MNIST_KD_B_LeNet.xlsx")
